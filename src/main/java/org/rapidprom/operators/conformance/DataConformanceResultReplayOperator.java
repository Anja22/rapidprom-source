...
public class DataConformanceResultReplayOperator extends Operator {
	...
	private OutputPort outputAlignment = getOutputPorts().createPort("alignment (ProM ResultReplay)");
...
//	private OutputPort outputMetricsDetailed = getOutputPorts().createPort("detailed metrics (Example set)");
	...
	public DataConformanceResultReplayOperator(OperatorDescription description) {
		super(description);
		...
		getTransformer().addRule(new GenerateNewMDRule(outputAlignment, ResultReplayIOObject.class));
//
//		addMetricsPort();
...
//					alignedLog, precisionConfig);
			...
			outputAlignment.deliver(new ResultReplayIOObject(alignment,context));
